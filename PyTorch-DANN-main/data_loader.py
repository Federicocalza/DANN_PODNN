...
def get_loader(args):
    if args.dset == 's2m':
        ...
        svhn_tr = transforms.Compose([transforms.Resize([32, 32]),
                                      transforms.ToTensor(),
                                      transforms.Normalize([0.5], [0.5])])
        t_train = datasets.SVHN(os.path.join(args.data_path, 'svhn'), split='train', download=True, transform=svhn_tr)
        t_test = datasets.SVHN(os.path.join(args.data_path, 'svhn'), split='test', download=True, transform=svhn_tr)
        ...
        t_train = datasets.ImageFolder(root=os.path.join(args.data_path, 'mnistm','training'), transform=mnistm_tr)
        t_test = datasets.ImageFolder(root=os.path.join(args.data_path, 'mnistm','testing'), transform=mnistm_tr)
